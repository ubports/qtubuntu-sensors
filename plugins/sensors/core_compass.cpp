...
void core::Compass::checkValues()
{
    ...
    if (Compass::getRotationMatrix(R, 9, I, 9, _gravity, _geomagnetic)) {
        Compass::getOrientation(R, 9, _orientation);
        if (_gyroscopeEnabled) {
...
void core::Compass::gyroFunction(QGyroscopeReading *event)
{
    ...
    if (_initState) {
        ...
        Compass::getOrientation(initMatrix, 9, test);
    ...
    Compass::getRotationMatrixFromVector(deltaMatrix, 9, deltaVector, 4);
    ...
    Compass::getOrientation(_gyroMatrix, 9, _gyroOrientation);
}
